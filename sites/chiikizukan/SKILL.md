...
| フィールド名 | 必須 | 種類 | 入力内容 |
|------------|------|------|--------|
| このプロジェクトでご自身の経験がどのように活かせそうですか？（200字以上） | 必須 | テキストエリア（200字以上） | COWORK_SKILL.md Step 4 のルールで生成した応募文。「はじめまして、むらりょう（中村崚）と申します。」から始め、my_profile.md の実績を1つ引用。地域DX・課題解決への貢献意欲も1文入れる。「ぜひ詳細をお聞かせください」で締める |
